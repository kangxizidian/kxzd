...
const isKangxi=window.location.href.indexOf('kx0')>-1;
...
const addWordheadLink=(domnode,wh)=>{
    ...
    lnk.className='backlink';
    lnk.innerText=String.fromCodePoint(wh);
    domnode.insertAdjacentElement('beforebegin',lnk);
}
const renderBacklinks=()=>{
    const anchors=document.getElementsByTagName('A');
    const m=window.location.href.match(/cct\/([^#]+)/);
    if (!m) return;
    const fn=m[1];

    const backlinks=kxzd_backlinks[fn];
    if (!backlinks)return;
    
    for (let i=0;i<anchors.length;i++) {
        const B=backlinks[anchors[i].name];
        if (!B)continue;
        const wordheads=B.split(/(u[\dA-F]+)/);
        let added=0;
        for (let j=0;j<wordheads.length;j++) {
            if (wordheads[j][0]!=='u')continue;
            if (!added) {
                const lnkgroup=document.createElement('span');
                lnkgroup.className='backlinkgroup';
                anchors[i].insertAdjacentElement('beforebegin',lnkgroup);
            }
            addWordheadLink(anchors[i], parseInt(wordheads[j].substr(1),16));
            added++;
        }
    }
}
...
if (window.location.hash && window.location.search.indexOf('embed')>-1) {
    ...
    loadScript('filewordhead.js')
    loadScript('backlinks.js')
    scrollToWH(unicode);
    setTimeout(()=>{
        renderBacklinks();
    },1000)
